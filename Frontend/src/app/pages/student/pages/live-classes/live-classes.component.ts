    ...
    <div class="space-y-6">
      ...
      <section *ngIf="liveNow.length > 0">
        ...
        <div class="grid grid-cols-1 gap-4">
          <div
            *ngFor="let session of liveNow"
            class="card border-2 border-red-500"
          >
            <div class="card-body">
              <div class="flex items-start justify-between">
                <div class="flex-1">
                  <span class="badge bg-red-100 text-red-800 text-xs mb-2">
                    <i class="fas fa-circle live-dot" aria-hidden="true"></i>
                    LIVE
                  </span>
            ...
            <div class="card-body">
              <div class="flex gap-4">
                <div class="flex-shrink-0 text-center">
                  <div
                    class="w-16 h-16 rounded-lg bg-primary-100 flex flex-col items-center justify-center"
                  >
                    <span class="text-2xl font-bold text-primary-600">{{
                      session.date | date: 'd'
                    }}</span>
                    <span class="text-xs text-primary-600">{{
                      session.date | date: 'MMM'
                    }}</span>
                  </div>
                </div>
                <div class="flex-1">
                  <span class="badge badge-primary text-xs mb-2">{{
                    session.course
                  }}</span>
                  ...
                  <div class="flex items-center gap-2 text-xs text-gray-500">
                    ...
                    {{ session.date | date: 'h:mm a' }} -
                    {{ session.duration }} min
                  </div>
                  <button class="mt-3 btn-outline w-full text-sm">
                    Set Reminder
                  </button>
                </div>
              </div>
      ...
      <section>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            <div
              class="relative h-40 bg-gray-900 rounded-t-xl overflow-hidden group"
            >
              <img
              ...
              <div class="absolute inset-0 flex items-center justify-center">
                <div
                  class="w-12 h-12 rounded-full bg-white/90 flex items-center justify-center group-hover:scale-110 transition-transform"
                >
                  <svg
                    class="w-6 h-6 text-primary-600 ml-1"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
            ...
            <div class="p-4">
              ...
              <p class="text-xs text-gray-500 mt-1">
                {{ recording.date | date: 'MMM d, yyyy' }}
              </p>
            </div>
          </div>
        </div>
