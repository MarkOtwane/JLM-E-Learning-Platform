  ...
  template: `
    <div class="space-y-6">
      ...
      <div class="page-header">
        <h1 class="page-title">{{ getGreeting() }}, {{ getStudentName() }}! 👋</h1>
      ...
      <section>
        <div class="flex items-center justify-between mb-4">
          ...
          <a routerLink="/student/courses" class="text-sm font-semibold text-primary-600 hover:text-primary-700">
            View All →
          </a>
        </div>
        ...
        <!-- Course Cards -->
        <div *ngIf="!isLoading && activeCourses.length > 0" 
        ...
        <div class="card">
          <div class="card-body">
            <div class="flex items-center justify-between mb-4">
              ...
              <a routerLink="/student/assignments" class="text-sm font-semibold text-primary-600 hover:text-primary-700">
                View All →
              </a>
            </div>

            <!-- Assignment List -->
            <div *ngIf="upcomingAssignments.length === 0" class="text-center py-8">
            ...
            <div class="space-y-3">
              <div *ngFor="let assignment of upcomingAssignments" 
                ...
                <div class="flex-1 min-w-0">
                  <h4 class="text-sm font-semibold text-gray-900 truncate">{{ assignment.title }}</h4>
                  <p class="text-xs text-gray-500 mt-0.5">{{ assignment.courseName }}</p>
                  <div class="flex items-center gap-2 mt-2">
                    <span class="text-xs font-medium" [ngClass]="getDueDateClass(assignment.dueDate)">
                      Due {{ formatDueDate(assignment.dueDate) }}
                    </span>
                  </div>
                </div>
                <span class="badge badge-warning text-xs">{{ assignment.status }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card">
          <div class="card-body">
            <div class="flex items-center justify-between mb-4">
              ...
              <a routerLink="/student/live-classes" class="text-sm font-semibold text-primary-600 hover:text-primary-700">
                View All →
              </a>
            </div>

            <!-- Live Class List -->
            <div *ngIf="upcomingLiveClasses.length === 0" class="text-center py-8">
              <svg class="w-12 h-12 mx-auto text-gray-400 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="space-y-3">
              <div *ngFor="let liveClass of upcomingLiveClasses" 
                ...
                <div class="flex-1 min-w-0">
                  <h4 class="text-sm font-semibold text-gray-900 truncate">{{ liveClass.title }}</h4>
                  <p class="text-xs text-gray-500 mt-0.5">{{ liveClass.instructor }}</p>
                ...
                <button class="btn-primary text-xs px-3 py-1.5 whitespace-nowrap">
                  Join
                </button>
              </div>
            </div>
          ...
          <div class="space-y-3">
            <div *ngFor="let announcement of announcements"
              ...
              <div class="flex-1">
                ...
                <p class="text-xs text-gray-500 mt-2">{{ announcement.date | date:'MMM d, yyyy' }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      this.announcements = [
        {
          ...
          message: 'Check out our new Machine Learning course now available in the catalog.',
          date: new Date()
        }
      ];
